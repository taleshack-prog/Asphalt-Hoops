...
router.get('/users/growth', async (_req, res: Response): Promise<void> => {
  try {
    const r = await pool.query(`
      SELECT TO_CHAR(d::date,'DD/MM') as label, COUNT(u.id) as count
...
router.get('/users/inviters', async (_req, res: Response): Promise<void> => {
  try {
    const r = await pool.query(`
      SELECT u.name, u.invite_code, COUNT(i.id) as invited_count
...
router.get('/users/most-active', async (_req, res: Response): Promise<void> => {
  try {
    const r = await pool.query(`
      SELECT u.name, COUNT(mp.match_id) as match_count
      FROM users u JOIN match_players mp ON mp.user_id=u.id
      GROUP BY u.id,u.name ORDER BY match_count DESC LIMIT 10`);
    res.json({ rows: r.rows });
  } catch(e){ res.status(500).json({error:'Erro interno'}); }
});
  ...
  try {
    const r = await pool.query(`
      WITH RECURSIVE tree AS (
        SELECT id, name, invite_code, invited_by, 0 as level
        FROM users WHERE invited_by IS NULL
        UNION ALL
        SELECT u.id, u.name, u.invite_code, u.invited_by, t.level+1
        FROM users u JOIN tree t ON u.invited_by=t.id
        WHERE t.level < 5
      )
      SELECT t.*, COUNT(i.id) as invited_count
      FROM tree t LEFT JOIN users i ON i.invited_by=t.id
      GROUP BY t.id,t.name,t.invite_code,t.invited_by,t.level
      ORDER BY t.level, t.name LIMIT 100`);
    res.json({ rows: r.rows });
  } catch(e){ res.status(500).json({error:'Erro interno'}); }
});
...
router.get('/matches/count', async (_req, res: Response): Promise<void> => {
  try {
    const r = await pool.query('SELECT COUNT(*) FROM matches');
...
router.get('/matches/modalities', async (_req, res: Response): Promise<void> => {
  try {
    const r = await pool.query('SELECT modality, COUNT(*) as count FROM matches GROUP BY modality ORDER BY count DESC');
    res.json({ rows: r.rows });
  } catch(e){ res.status(500).json({error:'Erro interno'}); }
});
...
router.get('/matches/all', async (_req, res: Response): Promise<void> => {
  try {
    const r = await pool.query(`
      SELECT m.id, c.name as court_name, m.modality, m.scheduled_at, m.max_players,
...
router.get('/courts/count', async (_req, res: Response): Promise<void> => {
  try {
    const r = await pool.query('SELECT COUNT(*) FROM courts');
    res.json({ count: +r.rows[0].count });
  } catch(e){ res.status(500).json({error:'Erro interno'}); }
});
...
router.get('/courts/user-added', async (_req, res: Response): Promise<void> => {
  try {
    const r = await pool.query(`
      ...
      WHERE c.added_by IS NOT NULL ORDER BY c.created_at DESC LIMIT 20`);
    res.json({ rows: r.rows });
  } catch(e){ res.status(500).json({error:'Erro interno'}); }
});
...
router.get('/courts/by-neighborhood', async (_req, res: Response): Promise<void> => {
  try {
    const r = await pool.query(`
      SELECT 
        SPLIT_PART(c.address,'—',2) as neighborhood,
        COUNT(DISTINCT c.id) as court_count,
        COUNT(m.id) as match_count
      FROM courts c LEFT JOIN matches m ON m.court_id=c.id
      GROUP BY neighborhood ORDER BY match_count DESC LIMIT 20`);
...
router.get('/messages/daily', async (_req, res: Response): Promise<void> => {
  try {
    const r = await pool.query(`
      SELECT TO_CHAR(d::date,'DD/MM') as date, COUNT(cm.id) as count
